...
# ====== 引数で設定 ======
if [ "$#" -ne 4 ]; then
  echo "使い方: $0 <SCOPE> <REGION> <TARGET_IP_FILE> <TARGET_IPSETS_FILE>"
  exit 1
fi

SCOPE="$1"               # 例: REGIONAL or CLOUDFRONT
REGION="$2"              # 例: ap-northeast-1
TARGET_IP_FILE="$3"      # 例: target_ips.txt
TARGET_IPSETS_FILE="$4"  # 例: target_ipsets.txt
# =====================
